...
struct texShader : public Shader {
  ...
  virtual Vec4f vertex(int iface, int nthvert) {
    ...
    Vec4f local_pos;
    local_pos << model_pos(0, nthvert), model_pos(1, nthvert),
        model_pos(2, nthvert), 1.0f;
  ...
  virtual bool fragment(Vec3f bary, TGAColor &color) {
    ...
    Vec2f uv0 = varying_uv.col(0), uv1 = varying_uv.col(1),
          uv2 = varying_uv.col(2);
    delta_uv << uv1[0] - uv0[0], uv1[1] - uv0[1], uv2[0] - uv0[0],
        uv2[1] - uv0[1];
    Eigen::Matrix<float, 2, 3> E;
    Vec3f p0 = model_pos.col(0), p1 = model_pos.col(1), p2 = model_pos.col(2);
    E << p1[0] - p0[0], p1[1] - p0[1], p1[2] - p0[2], p2[0] - p0[0],
        p2[1] - p0[1], p2[2] - p0[2];
    ...
    //  float shadow = 1.0f;
    //  if(!(s_pos[0] < 0.f || s_pos[0] > width || s_pos[1] < 0.f || s_pos[1] >
    //  width))
    //      shadow = .3+.7*(depthbuffer.get(s_pos[0], s_pos[1]).bgra[2] <=
    //      s_pos[2]);
    ...
    // for(int i = 0; i < 3; i++) color[i] = std::min<float>(5 +
    // color[i]*shadow*(diff + 0.6f * spec), 255);
    for (int i = 0; i < 3; i++)
      color[i] = std::min<float>(color[i] * (diff), 255);
    return false;
  }
};
...
//         Vec4f local_pos;
//         local_pos << model_pos(0, nthvert), model_pos(1, nthvert),
//         model_pos(2, nthvert), 1.0f; return Viewport * Projection * Modelview
//         * local_pos;
//     }
...
int main(int argc, char **argv) {
  if (2 == argc) {
    ...
    // model = new
    // Model("G:\\project\\c++\\tinyrender\\resource\\obj\\african_head\\african_head.obj");
    std::filesystem::path modelPath = std::filesystem::path("..") / "resource" /
                                      "obj" / "diablo3_pose" /
                                      "diablo3_pose.obj";
    std::cout << "default model path: " << modelPath << std::endl;
    std::cout << "Usage : ./tinyrender <obj file>\n";
    model = new Model(modelPath.c_str());
    // model = new
    // Model("G:\\project\\c++\\tinyrender\\resource\\obj\\floor.obj");
  }
